...
if [[ "$DEPLOYMENT_TARGETS" != "15.0" ]]; then
  echo "Deployment-target audit failed: expected only macOS 15.0, found: $DEPLOYMENT_TARGETS" >&2
  exit 1
fi
...
if [[ ! -d "$APP_PATH" ]]; then
  echo "Apple Silicon archive did not create the expected app bundle: $APP_PATH" >&2
  exit 1
fi

./script/verify_license_material.sh "$APP_PATH/Contents/Resources/Licenses" "$PRODUCT_VERSION"
codesign --verify --deep --strict --verbose=2 "$APP_PATH"
./script/verify_platform.sh "$APP_PATH"

echo "Tests, static analysis, and the Apple Silicon archive passed."
