...
struct ThunderboltPortRow: View {
                ...
                if let speed {
                    VStack(alignment: .trailing, spacing: 3) {
                        ...
                        if port.connectedDevice == nil, !isPowerSourceConnected,
                            let boost = port.bandwidthBoostSpeedMbps
                        {
                            Text("\(USBFormatting.transferRate(boost)) Boost")
                                .font(.caption2).foregroundStyle(.secondary)
                                .help("bandwidth_boost_explanation")
                        }
                    }
                }
            }
    ...
    private var detail: String {
        ...
        parts.append(port.connectedDevice?.transport == .usb ? "USB" : port.protocolDescription)
    ...
    private var speed: String? {
        ...
        if let maximum = port.standardMaximumSpeedMbps {
